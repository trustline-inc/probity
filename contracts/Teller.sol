...
contract Teller is ITeller, Ownable, ProbityBase {
   ...
   */
  function createLoan(address lender, address borrower, uint principal) external override onlyProbity {
    // TODO: Match with a lender
    treasury.decrease(principal, lender);
    balances[borrower] = balances[borrower].add(principal);
    treasury.transfer(borrower, principal);
    emit LoanCreated(lender, borrower, principal, block.timestamp);
  }
